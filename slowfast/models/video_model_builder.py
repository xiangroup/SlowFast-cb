...
class ResNet(nn.Module):
    ...
    def forward(self, x, x2, tymode, bboxes=None):
        if tymode == 'train':
            ...
            _, _, a = self.head(a_wr1)
            _, _, v1 = self.head(v_sr1)
            _, _, v2 = self.head(v_sr2)
            ...
            _, alpha3, _ = self.head(wr1bn)
            # alph3 = nn.BatchNorm2d(alph3)
            # alph3 = nn.ReLU(alph3)
            _, alpha4, _ = self.head(wr2bn)
